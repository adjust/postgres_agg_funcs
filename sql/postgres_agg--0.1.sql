...
CREATE OR REPLACE FUNCTION adeven_long_add(a hstore, b hstore) RETURNS hstore AS '/usr/local/lib/adjust/add.so' LANGUAGE C;
CREATE OR REPLACE FUNCTION adeven_float_add(a hstore, b hstore) RETURNS hstore AS '/usr/local/lib/adjust/float_add.so' LANGUAGE C;
